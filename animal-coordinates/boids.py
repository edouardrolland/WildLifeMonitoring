...
class Boid():
    ...
    def __init__(self, window, margin, x_initial, y_initial):
        
        if x_initial == None or y_initial== None:        
            ...
            self.stress = 0.0 #1.0 if the boid is stressed, 0.0 if the boid is chilling
        
        else:
            angle = np.random.uniform(0, 2 * np.pi)
            r = np.random.randint(0, (margin)*0.2)
            self.x, self.y = x_initial, y_initial
            self.vx = 0.0
            self.vy = 0.0
            self.vx_prev = 0
            self.vy_prev = 0
